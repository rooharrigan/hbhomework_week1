...
def all_odd(number_list):
    ...
    """
    odds_list = [i for i in number_list if i % 2 != 0]
    return odds_list
...
def all_even(number_list):
    ...
    evens_list = [i for i in number_list if i % 2 == 0]
    return evens_list
...
##Roo comment: not sure if this counts as passing becuase enumerate has a counter
##built into it. Check with Kai.
...
def long_words(word_list):
    ...
    """
    ##Assumes all values of the list are one word. 
    long_words = [i for i in word_list if len(i) > 4]
    return long_words
...
def halvesies(number_list):
    ...
    """
    halvesies = [(i / 2.0) for i in number_list]
    return halvesies
...
##Roo comment: wasn't sure if were supposed to use join, solved both ways but
##the second one is gross. Sorry about that.
...
def advanced_join_strings(list_of_words):
    ...
    If there's only one thing in the list, it should retur just that
    thing, of course:
...
def advanced_join_strings_without_join(list_of_words):
    """Return a single string with each word from the input list
    separated by a comma.
